...
public class IngressosController(IngressoService ingressoService) : ControllerBase
{
    ...
    [HttpPost("reembolso")]
    public async Task<IActionResult> Reembolsar([FromQuery] string codigoIngresso)
    {
        try
        {
            var (sucesso, mensagem, valorReembolsado) = await ingressoService.ReembolsarAsync(codigoIngresso);
            if (!sucesso) return NotFound(new { mensagem });
            return Ok(new { mensagem, valorReembolsado });
        }
        catch (Exception ex)
        {
            return StatusCode(500, new { mensagem = "Erro interno.", detalhe = ex.Message });
