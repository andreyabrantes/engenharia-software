...
import { ApiCategory, ApiResponse, ApiEvent } from '@/lib/api-types';
    ...
    async function fetchEvent() {
      try {
        ...
        if (data.success) {
          ...
          setCategoryId(String(ev.categoryId));
          setCity(ev.city);
          setLocation(ev.location);
          setAddress(ev.address);
          setCep(ev.cep || '');
          setStreet(ev.street || '');
          setNeighborhood(ev.neighborhood || '');
          setState(ev.state || '');
          setAddressNumber(ev.addressNumber || '');
          setDate(ev.eventDate.split('T')[0]);
          setTime(ev.time);
          ...
          if (ev.tickets && ev.tickets.length > 0) {
            setTickets(
              ev.tickets.map((t) => ({
                ...
                quantity: String(t.availableQuantity),
        ...
        <main className="flex flex-1 items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              {authLoading ? 'Verificando autenticação...' : 'Carregando evento...'}
            </p>
          </div>
        </main>
        <Footer />
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Dados principais do seu evento</CardDescription>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Altere a imagem de capa do seu evento
                </CardDescription>
              </CardHeader>
              <CardContent>
                {imageUrl && (
                  <div className="mb-4 overflow-hidden rounded-lg">
                    <img
                      src={imageUrl.startsWith('/') ? `${API_BASE_URL}${imageUrl}` : imageUrl}
                      alt="Preview"
                      className="h-40 w-full object-cover"
                    />
                  </div>
                )}
                <div className="flex items-center justify-center rounded-lg border-2 border-dashed border-border bg-secondary/30 p-8 transition-colors hover:border-primary/50">
