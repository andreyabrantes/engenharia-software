...
public class EventoService
{
    ...
    public async Task<(bool sucesso, string mensagem, decimal valorReembolsado)> ReembolsarAsync(string codigoIngresso)
    {
        try
        {
            var response = await _http.PostAsync(
                $"api/ingressos/reembolso?codigoIngresso={Uri.EscapeDataString(codigoIngresso)}", null);
            ...
            if (!response.IsSuccessStatusCode)
            {
                if (!string.IsNullOrWhiteSpace(body))
                {
                    try
                    {
                        var erro = System.Text.Json.JsonSerializer.Deserialize<MensagemErro>(body,
                            new System.Text.Json.JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                        return (false, erro?.Mensagem ?? "Erro ao processar reembolso.", 0);
                    }
                    catch { }
                }
                return (false, "Erro ao processar reembolso.", 0);
            }

            if (!string.IsNullOrWhiteSpace(body))
            {
                var resultado = System.Text.Json.JsonSerializer.Deserialize<ReembolsoResultado>(body,
                    new System.Text.Json.JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                OnEstadoAlterado?.Invoke();
                return (true, resultado?.Mensagem ?? "Reembolso realizado.", resultado?.ValorReembolsado ?? 0);
            }

            OnEstadoAlterado?.Invoke();
            return (true, "Reembolso realizado com sucesso.", 0);
        }
        catch (Exception ex)
        {
            return (false, $"Erro ao processar reembolso: {ex.Message}", 0);
        }
    }
    ...
    private sealed class ReembolsoResultado
    {
        public string Mensagem { get; set; } = string.Empty;
